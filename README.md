...
| `hive team` | Show team with runtime `busy` / `inputState` / peer info + per-member `group` (from `@hive-group` tag); `self` is a string pointer to your member name — look yourself up in `members`. In tmux with no team bound, returns a bootstrap payload (`team: null`, tmux panes, `hint`). |
