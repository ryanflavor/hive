...
在 Claude Code / Codex 里，请通过 shell escape 调用这些命令，例如：`!hive cvim`。

其它命令，例如 `hive send`、`hive reply`、`hive team`、`hive doctor <agent>`、`hive handoff`、`hive fork` 等，都是按“由 agent 调用”来设计的。你手动运行也可以，但那属于调试 / 高阶路径，不是默认 happy path。
