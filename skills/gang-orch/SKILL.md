...
`selfMember.name` 形如 `<gang>.orch`(例:`peaky.orch` / `shelby.orch`),`group` 等于同一个 `<gang>`。**`.` 之前的前缀就是你的 gang 实例名**,下文全部 `<gang>` 占位符都用这个值替换(例:你是 `peaky.orch` → `<gang>.skeptic` = `peaky.skeptic`)。
