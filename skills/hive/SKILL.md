...
- **消息机制** — 怎么收、怎么发、thread / root 协议 / busy-fork / shell 安全
...
**原文永远在 `<HIVE>` block 里读。** `hive thread <msgId>` 和 `hive delivery <msgId>` 是排障入口(见 Debug 章节),agent 日常收信用不上。
